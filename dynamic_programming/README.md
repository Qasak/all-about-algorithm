...
+ 首先解决基case
  + 对于每个节点$v$, $D_{\{v\},v}=0$
+ 递推关系
  + 考虑一条路径访问$S$种的所有顶点恰好一次并停在$v$
  + 在到达$v$之前，路径从集合$S-\{v\}$中的某个$u$到来
  + 这条子路径一定是覆盖集合$S-\{v\}$,停在$u$的路径中最优的
  + 我们尝试所有候选的$u$

$$
D_{S,v}=\min_{u \in S-\{v\}}
\left(
\begin{matrix}
D_{S-\{v\},u}+cost(u,v)
\end{matrix}
...
+ 当处理子集时，最好找到一个舒服的表达集合的方式
+ Idea：用一个整数表达一个集合
  + 用小整数$\{0,1,...\}$来表示子集
  + eg.19=$010011_{2}$表示集合$\{0,1,4\}$.(小端序)
